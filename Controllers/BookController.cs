...
namespace Seat_Reservation_System.Controllers
{
    public class BookController : Controller
    {
		...
		public IActionResult GetUsersById(string username) {
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, conn))
                {
					...
					{
						var user = new User();  // Assuming Booking is your model class
                        while (reader.Read())
                        {
                            // Populate the bookings list with the data from the database
							user.TraineeName = reader["TraineeName"].ToString();
							user.TraineeNIC = reader["TraineeNIC"].ToString();
		...
		{
			var bookings = new List<Bookings>();
			string query = "SELECT SeatNumber FROM Booking WHERE BookDateTime = @date AND isExpire = 0";
		...
		public async Task<IActionResult> AddRequest([FromBody] Bookings data)
		{
			...
			string query = "INSERT INTO Booking VALUES (@TraineeId,@TraineeName,@BookDateTime,@TraineeNIC,@SeatNumber,0)";
							...
							using (SqlCommand selectCommand = new SqlCommand(selectQuery, conn))
							{
								...
								using (SqlDataReader reader = selectCommand.ExecuteReader())
								{
									...
									while (reader.Read())
									{
										...
										booking.TraineeName = reader["TraineeName"].ToString();
										booking.TraineeNIC = reader["TraineeNIC"].ToString() ;
		...
		public async Task<IActionResult> GetBookingsByDate(string date,string traineeid)
		{
			var bookings = new List<Bookings>();
			//traineeid = "3";
			string query = "SELECT * FROM Booking WHERE BookDateTime = @date AND TraineeId = @traineeid AND isExpire = 0";
        ...
        [HttpPost]
        //[Route("/UpdateCount")]
        public IActionResult UpdateCount(string username)
        {
            
            string query = "UPDATE Users SET LeaveCount = 1 WHERE Username = @Username";
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                conn.Open();
                using (SqlCommand command = new SqlCommand(query, conn))
        ...
        {
            // Define the query to select LeaveCount
            string query = "SELECT LeaveCount FROM Users WHERE Username = @Username";
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, conn))
                {
                    ...
                    // Check if result is null (username not found)
                    if (result == null)
                    {
                    ...
                    int leaveCount;
                    if (int.TryParse(result.ToString(), out leaveCount))
                    {
                        return Ok(leaveCount);
                    }
                    else
                    {
                        return StatusCode(500, "Error parsing the leave count.");
                    }
                }
            }
        }
